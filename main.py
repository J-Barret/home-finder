...
def read_ranges_from_json():
	with open("settings.json", 'r') as file:
		...
		return price_range, rooms_range, size_range, email, proxy_protection

def function_with_proxy(price_range, rooms_range, size_range, email):
	...
	while True:
		...
		if(proxy_ip is not None and proxy_port is not None):
			if (idealista_call(proxy_ip=proxy_ip,proxy_port=proxy_port) is True):
				idealista_parser()
...
def function_without_proxy(price_range, rooms_range, size_range, email):
	print("RUNNING WITHOUT PROXY... Press 'q' if you want to exit the program")
	#-------------------------MAIN LOOP --------------------------------------------
	while True:
		if (idealista_call() is True):
			idealista_parser()
			evaluate_new_houses(price_range=price_range, rooms_range=rooms_range, size_range=size_range)
			new_houses_urls = get_new_urls()
			if new_houses_urls:
...
def main():
	price_range, rooms_range, size_range, email, proxy_protection = read_ranges_from_json()
	if(proxy_protection == "yes"):
		function_with_proxy(price_range, rooms_range, size_range, email)
	elif(proxy_protection == "no"):
		function_without_proxy(price_range, rooms_range, size_range, email)
	else:
